...
# Verificar si la columna 'ubicacion' no existe y agregarla si es necesario
cursor.execute('''PRAGMA table_info(ventanas)''')
columns = cursor.fetchall()
ubicacion_column_exists = any(col[1] == 'ubicacion' for col in columns)
if not ubicacion_column_exists:
    cursor.execute('''ALTER TABLE ventanas ADD COLUMN ubicacion TEXT''')
    conn.commit()
...
def sugerir_ventanas_no_utilizadas(ubicacion_html, tiempo_limite=600, ventanas_por_pagina=1):
    ...
    # Agregar ubicación HTML al PDF
    pdf.set_font("Arial", size=12)
    pdf.cell(0, 10, txt=f"Ubicacion desde la cual se envia el pdf: {ubicacion_html}", ln=True)
    pdf.ln(10)  # Agregar espacio
    ...
    for ventana in ventanas_no_utilizadas:
        ...
        cursor.execute(
            "INSERT OR REPLACE INTO ventanas (ventana, tiempo_inactividad, tiempo_total_abierta, ubicacion) VALUES (?, ?, ?, ?)",
            (ventana.encode('latin-1', 'replace').decode('latin-1'), tiempo_formateado_inactividad,
             tiempo_formateado_total_abierta, ubicacion_html))
    ...
    pdf_output_path = f"C:\\Users\\jdvir\\OneDrive\\Escritorio\\{pdf_file}"
    pdf.output(pdf_output_path)
    print(f"Se ha creado el archivo PDF en: {pdf_output_path}")

def extraer_ubicacion_html(filename):
    """Abre un archivo HTML y extrae la ubicación."""
    # Ruta del archivo HTML
    filepath = 'C:\\Users\\jdvir\\OneDrive\\Escritorio\\' + filename

    # Iniciar el navegador controlado por Selenium (para Edge)
    driver = webdriver.Edge()

    # Abrir el archivo HTML en el navegador controlado por Selenium
    ...
    # Extraer la ubicación del HTML
    ubicacion = soup.find(id='ubicacion').text.strip() if soup.find(id='ubicacion') else None

    return ubicacion

if __name__ == "__main__":
    ubicacion_html = extraer_ubicacion_html("UBICACION.html")
    ubicacion = ubicacion_html if ubicacion_html else "No se pudo extraer la ubicación"
    sugerir_ventanas_no_utilizadas(ubicacion)
